  ...
  return (
    <div className="flex flex-col items-center py-8">
      <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-6 py-4">
        {techStack
          .filter((tech) => tech.trim() !== '') // Filter empty strings
          .map((tech, index) => {
                ...
                <div className="w-12 h-12 lg:w-14 lg:h-14 relative">
                  <Image
                    ...
                    onError={(e) => {
                      ...
                      // Prevent infinite loop: if current src includes the fallback extension, stop.
                      if (target.src.endsWith('.png')) {
                        target.style.display = 'none'; // Hide if both fail
                      } else {
                        target.src = iconPaths[1];
                      }
                    }}
                  />
                </div>
